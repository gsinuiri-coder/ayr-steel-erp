...
if (branch === 'production') {
  throw new Error('Este backfill no se ejecuta contra production desde la sesión; ensayar en la rama clon.');
}
...
const result = spawnSync(process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm', ['exec', 'tsx', 'prisma/backfill-seller-scope.ts', ...childArgs], {
  cwd: resolve(ROOT, 'apps/api'), env, stdio: 'inherit', shell: process.platform === 'win32',
});
