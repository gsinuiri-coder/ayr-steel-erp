...
| 7e — Venta de bobinas + catálogo estructurado + cotización | 🟡 En curso (2026-09-06) | A+B+C+D+E hechos (D-116..D-120) y D-121 (pestañas de `/bobinas`, piezas teóricas en planta), aprobados por el dueño. D-122 (sacar el `ProductBom` de coberturas) diseñado y diferido al tramo 7e-ii. Primer push a CI encontró 25 fallas reales en specs de fases anteriores (fase1/2b/3/3b/5a/6/7b/7d) que nunca se corrieron contra el código completo de esta fase: catálogo estructurado (D-118) exige campos que esos tests no llenaban, bobinas nacen `CLOSED` (D-117) y esos tests asumían `OPEN`, el corte tercerizado es solo Drywall (D-120) y algunos casos de borde usaban una bobina de Metallic Roofing, y la compra de bobinas es solo Drywall/Metallic Roofing (D-117) y un test usaba `trading`. Las 9 specs corregidas, 58/58 verde en local (la suite completa de fase7b-bordes no corrió limpia en local por una caja de POS que quedó abierta de pruebas anteriores en `dev` — residuo esperado, no reproduce en `ci`, que resetea). Push de la corrección en curso. Falta: CI verde, deploy, e2e:prod, purga, handoff. |
