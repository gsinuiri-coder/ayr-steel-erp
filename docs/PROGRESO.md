...
| 8   | Tests unit (Jest) + E2E Playwright                           | ✅ 23 unit; 7 E2E en local (Neon `dev`); 3 E2E de lectura verdes contra producción        |
...
- Login real del administrador verificado contra producción (cookies `httpOnly`/`Secure`/`SameSite` correctas) y 3 tests E2E de solo lectura (login correcto, login fallido, logout) en verde contra `https://ayr-steel-erp-web.vercel.app`.
