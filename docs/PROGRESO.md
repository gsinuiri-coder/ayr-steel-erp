...
| 7e — Venta de bobinas + catálogo estructurado + cotización | 🟡 En curso (2026-09-06) | A+B+C+D+E hechos (D-116..D-120): venta de bobina completa, defaults de bobina, catálogo estructurado, cotización/pedido multi-línea + duplicar, corte solo Drywall + plan por SKU de fleje. 8/8 E2E nuevos verdes. Revisión de `revisor`/`auditor-seguridad` sin bloqueantes. Revisión local del dueño: pidió dos ajustes de alcance (D-121, pestañas de `/bobinas` y piezas teóricas en planta) — implementados, lint/typecheck/test/build verde, verificación de qa en curso. También pidió sacar el `ProductBom` de coberturas (D-122): diseño completo documentado, **implementación diferida al tramo 7e-ii** por alcance (toca una columna `NOT NULL`, cuatro archivos de backend y el DTO). Falta: verificación de D-121, revisión local final del dueño, push, deploy, e2e:prod. |
