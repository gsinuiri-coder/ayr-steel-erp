...
## Estado tras las correcciones (2026-09-24)

Las corrigió la misma sesión de Claude Code que escribió este informe: **el revisor pasó a ser el
autor del delta**. Por eso el delta necesita su propio repaso en una sesión nueva (AGENTS.md §2.2).
Cada P1 lleva un test que **falla con el código anterior** (comprobado corriéndolo contra
`2455291`) y pasa con el nuevo.

| Hallazgo | Qué se hizo                                                                                                                                                                                                                                                                                                                                                                                                                                 | Commit    |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| P1-1     | **Corregido** (D-258). Emparejamiento por producto normalizado + cantidad, con el importe como desempate. Sin pareja única, la línea va a (c) y no se toca. Una línea común nunca se convierte en bobina. El dry-run muestra SKU actual → nuevo y el del papel. 5 tests nuevos, en rojo contra el código anterior.                                                                                                                          | `a719f8e` |
| P1-2     | **Corregido** (D-259). `run-api-cli.mjs` fuerza cola, PSE y R2 apagados. Cada CLI imprime las banderas y aborta si alguna quedó encendida. Sin R2, la cotización que se corrige queda sin `pdfKey` y su PDF se arma al vuelo. Tests: `external-outputs.spec.ts`, `run-api-cli.test.mjs`, `quotation-pdf-stale.spec.ts`.                                                                                                                     | `5f5b320` |
| P1-3     | **Corregido** (aclaración de D-256). Exención y venta parcial solo para ADMINISTRADOR. Los importes del papel se conservan para cualquier rol mientras la línea no cambie. La marca `Factura externa:` ya no se puede tipear. E2E con un VENDEDOR real (`piso-importado-por-rol-d256.spec.ts`): 3/3, en rojo contra el código anterior.                                                                                                     | `a719f8e` |
| P1-4     | **Corregido** (D-260). `normalize:coil-skus --revert` desde la auditoría, con dry-run por defecto. E2E de ida y vuelta: estado inicial restaurado y reportes de RF-S4a iguales al centavo. La normalización queda como punto crítico en el runbook del handoff.                                                                                                                                                                             | `c09984a` |
| P2-1     | **Corregido.** El servidor valida el pool también en el importador (contra el producto de la fila) y en la edición de cotizaciones (contra los pools del propio documento): identidad, reservas, OP y otra cotización abierta.                                                                                                                                                                                                              | `a719f8e` |
| P2-3     | **Corregido.** El dry-run lista la parada de «producto a unir con saldo o reservas propias».                                                                                                                                                                                                                                                                                                                                                | `59ef1d5` |
| P2-5     | **Corregido.** La corrección de cotizaciones del barrido lleva su motivo a la auditoría.                                                                                                                                                                                                                                                                                                                                                    | `a719f8e` |
| P2-7     | **Corregido.** El rechazo a desactivar un `BOB…` también cuenta las bobinas `CLOSED` con saldo.                                                                                                                                                                                                                                                                                                                                             | `59ef1d5` |
| P2-9     | **Documentado** en el código: el IGV cero no pasa `paperTriplet`, y ±0.01 por línea es la tolerancia. No se aprieta: el papel redondea a tres decimales.                                                                                                                                                                                                                                                                                    | `59ef1d5` |
| P2-10    | **Corregido.** `check:coil-skus` usa `coilSaleSkus`, la misma cuenta que la venta.                                                                                                                                                                                                                                                                                                                                                          | `59ef1d5` |
| P2-2     | **No se corrige.** Con cantidades parciales, dos pedidos pueden reservar partes del mismo rollo. El kardex está protegido por el lock de `reserveLines`. Cerrarlo exige tocar ese lock, compartido por cinco módulos, y no es barato antes de la ventana. En cotizaciones, ahora el servidor sí rechaza la bobina tomada por otra cotización abierta (P2-1).                                                                                | —         |
| P2-4     | **No se corrige.** El execute sigue recalculando el plan en vez de exigir el que se aprobó. Mitigación en el runbook: dry-run y execute uno detrás del otro, en horario muerto, y comparar las cifras de salida de los dos.                                                                                                                                                                                                                 | —         |
| P2-6     | **No se corrige.** Los decimales de `subtotal`/`igv`/`total` hacia Nubefact. El PSE está apagado en producción; pendiente `pnpm e2e:pse` en demo antes de encenderlo.                                                                                                                                                                                                                                                                       | —         |
| P2-8     | **No se corrige.** La detección de filas de bobina por la palabra «BOBINA» en la descripción es regla de D-252 («la descripción, de respaldo»). Cambiarla es decisión del dueño, no un arreglo barato.                                                                                                                                                                                                                                      | —         |
| P2-11    | **No se corrige.** Sacar `/tmp/` del `.gitignore` expondría a un commit accidental los exports reales que el dueño deja ahí. Queda para que el dueño decida: moverlos a `local-data/` o registrar la excepción.                                                                                                                                                                                                                             | —         |
| P2-12    | **Hecho** (2026-09-24): ensayo completo sobre `demo` repuesta desde production. Normalizar → barrer → revertir → re-normalizar dejó los reportes de RF-S4a idénticos en las cinco fotos, y el catálogo `BOB…` quedó exacto tras revertir. Encontró tres defectos, corregidos: la CLI no arrancaba contra Neon, lo cerrado competía por la bobina y la transacción de 5 s. También una decisión pendiente (el trío del papel). Ver PROGRESO. | —         |
