...
| D-015 | 2026-09-02 | El web consume el API por rewrite same-origin `/api/*` → `API_URL` (`next.config.ts`). Cookies httpOnly `SameSite=Lax` sin `domain`. CORS del API solo para `WEB_ORIGIN`. Detalle en `DECISIONES.md`. | Evita cookies de terceros entre Vercel y Cloud Run.                                |
