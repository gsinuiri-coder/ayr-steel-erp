...
- **Web y API van juntos (HOTFIX-DESFASE, 2026-09-16).** Toda ventana cierra verificando que el label `git-sha` de la revisión activa de Cloud Run == SHA de `origin/main`. Deploy de API siempre con label `git-sha` (`--update-labels git-sha=<sha corto>`): un push a `main` publica el web en Vercel solo, la API no.
  - `pnpm smoke:prod` se corre desde un worktree en el **mismo SHA que se desplegó**, nunca desde `main` local: un smoke más nuevo que la API da falsos rojos, y uno más viejo, falsos verdes.
  - `migrate diff` no vacío en una ventana: se permite solo si el drift coincide **exactamente** con el ya documentado (`docs/PROGRESO.md`, «Incidente HOTFIX-DESFASE», deuda S3); cualquier diferencia nueva → PARA.
  - **Regla git-sha (b), ventana RF-S1+HOTFIX (2026-09-16).** El label no tiene que quedar igual a `HEAD` de `main` para siempre — el cierre de la ventana suele agregar commits de solo documentación (`docs/`, `.claude/`) **después** del deploy. Lo que se verifica es que no haya drift en las rutas de runtime: `git diff --quiet <sha desplegado> origin/main -- apps packages Dockerfile .gcloudignore package.json pnpm-lock.yaml pnpm-workspace.yaml`. Exit code 0 → cierra igual aunque el label ya no sea el último commit de `main`. Exit code 1 → el label está desfasado de código real, no solo de docs: PARA.
...
5. **Secretos**: `.env.setup` tiene todas las credenciales. Nunca imprimirlo, nunca commitearlo, nunca copiar valores a código o docs. Los scripts lo leen con `scripts/lib.mjs#readEnvFile`.
   ...
   - **Comandos con credenciales de BD de prod, solo con el dueño fuera de modo automático (ventana RF-S1+HOTFIX, 2026-09-16).** `migrate deploy`/`migrate diff`/`db:prod` contra `production` corren con el dueño en modo manual, aprobando cada ejecución una por una — nunca en modo automático, ni siquiera dentro de una ventana ya autorizada de punta a punta. El agente puede proponer el comando siguiente sin esperar más contexto, pero no encadena varios de estos sin que cada uno pase por su propia aprobación.
